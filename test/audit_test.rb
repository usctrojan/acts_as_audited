...
class AuditTest < Test::Unit::TestCase
  def setup
    @business = User.new :name => "testing"
    @audit = Audit.new
  end
  ...
  context "reconstruct_attributes" do
    should "work with with old way of storing just the new value" do
      audits = Audit.reconstruct_attributes([Audit.new(:revisions => {'attribute' => 'value'})])
  ...
  context "new_attributes" do
    should "return a hash of the new values" do
      Audit.new(:revisions => {:a => [1, 2], :b => [3, 4]}).new_attributes.should == {'a' => 2, 'b' => 4}
    end
  end

  context "old_attributes" do
    should "return a hash of the old values" do
      Audit.new(:revisions => {:a => [1, 2], :b => [3, 4]}).old_attributes.should == {'a' => 1, 'b' => 3}
    end
  end
  ...
  context "as_user" do
    ...
    should "record usernames" do
      Audit.as_user(@business.name) do
        company = Company.create :name => 'The auditors'
        company.name = 'The Auditors, Inc'
        company.save
    ...
    should "be thread safe" do
      begin
        t1 = Thread.new do
          Audit.as_user(@business) do
            sleep 1
            Company.create(:name => 'The Auditors, Inc').audits.first.user.should == @business
          end
        end

        t2 = Thread.new do
          Audit.as_user(@business.name) do
            Company.create(:name => 'The Competing Auditors, LLC').audits.first.username.should == @business.name
            sleep 0.5
          end
        end
